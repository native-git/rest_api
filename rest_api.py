...
global server, location_port, target_port
...
def api_location():
	...
	try:
		...
		yaw = rot[2]
		s.close()
		return 'X: ' + str(x) + " Y: " + str(y) + " THETA: " + str(roll)
...
def go_to():
	...
	roll = request.args.get('r', type=float)

	s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	s.connect((server,target_port))

	# Receive the auto-replied position information
	msg = str(x)+","+str(y)+","+str(0.0)+":"+str(roll)+","+str(0.0)+","+str(0.0)+"\n"
	...
	s.close()
	return 'Setting New Target-<br/>X: ' + str(x) + " Y: " + str(y) + " THETA: " + str(roll)
